...
int t_init(int width, int heigth, screenMode sm)
{
    ...
    bitmap=(color*)calloc(bitmapSize,1);
    fillmapSize=scr_w*scr_h*sizeof(bool);
    fillmap=(bool*)malloc(fillmapSize);
...
bool circle_filled_pixel_code(int i, int j, double center_x, double center_y, double r, double rr)
{
...
void line(double x1, double y1, double x2, double y2, bool needTurn)
{
...
void move(double x1, double y1, double x2, double y2, bool needTurn)
{
    int ji=0;
    double w=(y2-y1);
    double h=-(x2-x1);
...
void goTo(double x, double y, bool needTurn)
{
...
bool fillFilled(int x,int y)
{
    bool* v=fillmap+x+y*scr_w;
    bool res=*v;
    *v=false;
    return res;
}
